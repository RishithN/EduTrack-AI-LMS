    ...
    if (loading) {
        return (
            <div className="text-center py-20">
                ...
                <p className="text-slate-600 dark:text-slate-400 mt-4">Loading curated resources...</p>
            </div>
        );
    }
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">Filter Resources</h3>
                </div>
                <div className="grid md:grid-cols-3 gap-4">
                    <select
                        value={filters.difficultyLevel}
                        onChange={(e) => setFilters({ ...filters, difficultyLevel: e.target.value })}
                        className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white px-4 py-2 rounded-lg focus:outline-none focus:border-blue-500"
                    >
            ...
            {resources.length === 0 ? (
                <div className="text-center py-20">
                    ...
                    <p className="text-slate-600 dark:text-slate-400">No resources found. Try adjusting your filters.</p>
                </div>
            ) : (
                <div className="grid md:grid-cols-2 gap-4">
                    {resources.map((resource, idx) => (
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl hover:border-blue-500/50 hover:bg-slate-50 dark:bg-slate-800 transition-all group"
                        >
                                ...
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${resource.type === 'course' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                                        resource.type === 'video' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' :
                                            resource.type === 'certification' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                                                'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 border border-slate-600'
                                    }`}>
                                    {resource.type}
                                </span>
                            ...
                            <h3 className="font-bold text-lg text-slate-900 dark:text-white group-hover:text-blue-400 transition-colors mb-2">
                                {resource.title}
                            </h3>
                            ...
                            <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                                <div className="flex flex-wrap gap-2">
                                    <span className="px-2 py-1 bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 rounded text-xs border border-slate-200 dark:border-slate-800">
