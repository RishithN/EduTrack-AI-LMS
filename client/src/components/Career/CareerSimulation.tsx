...
const CareerSimulation = ({ studentId, roleId }: CareerSimulationProps) => {
    ...
    if (loading) {
        return (
            <div className="text-center py-20">
                ...
                <p className="text-slate-400 mt-4">Simulating your career trajectory...</p>
            </div>
        );
    }
    ...
    if (!simulation) {
        return (
            <div className="text-center py-20">
                <p className="text-slate-400">Unable to load career simulation.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Simulation Card */}
            <div className="bg-gradient-to-br from-slate-900 to-slate-900/50 border border-slate-800 rounded-2xl p-8 space-y-6">
                {/* Job Role */}
                <div className="text-center pb-6 border-b border-slate-800">
                    ...
                    <h2 className="text-4xl font-bold text-white mb-2">{currentData.jobRole}</h2>
                ...
                {currentData.salaryRange && (
                    <div className="bg-slate-950/50 border border-slate-800 p-6 rounded-xl">
                        ...
                        <p className="text-sm text-slate-400 mt-1">{currentData.salaryRange.experience}</p>
                    </div>
                )}
                ...
                <div className="bg-slate-950/50 border border-slate-800 p-6 rounded-xl">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-lg font-bold text-white">Skills You'll Have</h3>
                    </div>
                ...
                <div className="bg-slate-950/50 border border-slate-800 p-6 rounded-xl">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-lg font-bold text-white">Projects & Experience</h3>
                    </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
                <h3 className="text-lg font-bold text-white mb-4">Career Progression Timeline</h3>
                    ...
                    <div className="flex justify-between relative">
                        {[
                            ...
                            <div key={idx} className="flex flex-col items-center">
                                <div className={`w-12 h-12 rounded-full ${activeYear === item.year ? 'bg-blue-500' : 'bg-slate-800'
                                    } border-4 border-slate-950 flex items-center justify-center text-white font-bold mb-2 cursor-pointer transition-all hover:scale-110`}
                                    onClick={() => setActiveYear(item.year as 1 | 3 | 5)}
                                >
                                    {item.year}
                                </div>
                                <div className="text-center">
                                    <div className="text-sm font-semibold text-white">{item.data.jobRole}</div>
