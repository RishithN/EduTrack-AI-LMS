...
const DynamicRoadmap = ({ studentId, roleId }: DynamicRoadmapProps) => {
    ...
    if (loading) {
        return (
            <div className="text-center py-20">
                ...
                <p className="text-slate-600 dark:text-slate-400 mt-4">Generating your personalized roadmap...</p>
            </div>
        );
    }
    ...
    if (!roadmap) {
        return (
            <div className="text-center py-20">
                <p className="text-slate-600 dark:text-slate-400">Unable to load roadmap.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8">
            {/* Roadmap Header */}
            <div className="bg-gradient-to-r from-blue-900/50 to-purple-900/50 border border-slate-300 dark:border-slate-700 p-6 rounded-xl">
                <div className="flex items-start justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Your Learning Roadmap</h2>
                        <p className="text-slate-700 dark:text-slate-300">Personalized path to become a {roadmap.roleName}</p>
                    </div>
                ...
                <div className="mt-4 w-full h-2 bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden">
                    <div
            ...
            {/* Roadmap Phases */}
            <div className="space-y-6 relative before:absolute before:left-[19px] before:top-2 before:h-full before:w-0.5 before:bg-slate-50 dark:bg-slate-800">
                    ...
                    <div key={phaseIdx} className="relative">
                        ...
                        <div className="flex gap-4">
                            <div className={`w-10 h-10 rounded-full ${phase.status === 'completed' ? 'bg-green-500' :
                                ...
                                } border-4 border-slate-950 flex items-center justify-center shrink-0 z-10 text-slate-900 dark:text-white font-bold`}>
                            ...
                            <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6">
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-1">{phase.phaseName}</h3>
                                    ...
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${phase.status === 'completed' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                                            phase.status === 'in-progress' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                                                'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-slate-700'
                                        }`}>
                                ...
                                <div className="space-y-3">
                                    <h4 className="font-bold text-slate-700 dark:text-slate-300 text-sm">Skills to Master:</h4>
                                    <div className="grid md:grid-cols-2 gap-3">
                                        {phase.skills?.slice(0, 4).map((skill: any, skillIdx: number) => (
                                            <div key={skillIdx} className="bg-slate-950/50 border border-slate-200 dark:border-slate-800 p-3 rounded-lg">
                                                <div className="flex items-start justify-between mb-2">
                                                    <span className="text-slate-900 dark:text-white font-medium text-sm">{skill.skillName}</span>
                                ...
                                {phase.milestones && phase.milestones.length > 0 && (
                                    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                                        <h4 className="font-bold text-slate-700 dark:text-slate-300 text-sm mb-2">Milestones:</h4>
                                        <div className="space-y-2">
                                            {phase.milestones.map((milestone: any, mIdx: number) => (
                                                <div key={mIdx} className="flex items-center gap-2 text-sm">
                                                    ...
                                                    <span className={milestone.completed ? 'text-slate-600 dark:text-slate-400 line-through' : 'text-slate-700 dark:text-slate-300'}>
                                                        {milestone.name}
                                                    </span>
                                                </div>
                                            ))}
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Timeline Estimate</h3>
                <div className="grid md:grid-cols-3 gap-4">
                    <div className="text-center">
                        <div className="text-3xl font-bold text-blue-400">{roadmap.estimatedTotalMonths}</div>
                        <div className="text-sm text-slate-600 dark:text-slate-400">Total Months</div>
                    </div>
                    <div className="text-center">
