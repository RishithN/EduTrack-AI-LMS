...
const CareerResults = ({ results, onSelectRole }: CareerResultsProps) => {
    ...
    if (!topMatch) {
        return (
            <div className="text-center py-20">
                <p className="text-slate-400">No career matches found.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-gradient-to-br from-blue-900/50 via-purple-900/50 to-pink-900/50 border border-slate-700 p-8 rounded-2xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500" />
                <div className="absolute top-4 right-4">
                    <Award className="text-yellow-400" size={32} />
                </div>

                <h2 className="text-slate-400 uppercase tracking-widest text-sm font-bold mb-2">🎯 Top Career Match</h2>
                <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4">{displayMatch.roleName}</h1>
            ...
            {matches.length > 1 && (
                <div>
                    <h3 className="text-xl font-bold text-white mb-4">Other Strong Matches</h3>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {matches.slice(1, 4).map((match: any, idx: number) => (
                            <div key={idx} className="bg-slate-900 border border-slate-800 p-6 rounded-xl hover:border-blue-500/50 transition-all group">
