            ...
            <div className="space-y-8">
                ...
                <div className="bg-gradient-to-r from-yellow-600/20 to-orange-600/20 border border-yellow-500/30 rounded-2xl p-8 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex items-center gap-6">
                        <div className="h-24 w-24 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 border-4 border-slate-900 shadow-xl flex items-center justify-center text-4xl font-bold text-white">
                            5
                        </div>
                        <div>
                            <h2 className="text-sm font-bold text-yellow-400 uppercase tracking-widest mb-1">Current Level</h2>
                            <h1 className="text-4xl font-extrabold text-white">Intermediate Coder</h1>
                            ...
                            <div className="w-full max-w-sm h-2 bg-slate-800 rounded-full mt-3 overflow-hidden">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {BADGES.map((badge, idx) => (
                                <div key={idx} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center gap-4 hover:border-purple-500/50 transition-colors">
                                    <div className="text-3xl">{badge.icon}</div>
                                    <div>
                                        <h4 className="font-bold text-slate-200">{badge.name}</h4>
                            ...
                            <button
                                ...
                                className="border border-dashed border-slate-700 bg-slate-900/50 rounded-xl flex flex-col items-center justify-center text-slate-500 p-4 gap-2 hover:bg-slate-800 transition-all group"
                            >
                                <div className="h-10 w-10 rounded-full bg-slate-900 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    {showLocked ? '⬆️' : '?'}
                                </div>
                            ...
                            <div className="mt-6 mb-2 animate-in slide-in-from-top-4 fade-in duration-300">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {LOCKED_BADGES.map((badge, idx) => (
                                        <div key={idx} className="bg-slate-950/50 border border-slate-800/50 p-4 rounded-xl flex items-center gap-4 opacity-50 grayscale hover:grayscale-0 hover:opacity-100 transition-all cursor-not-allowed">
                                            <div className="text-3xl filter blur-[2px] hover:blur-none transition-all">{badge.icon}</div>
                                            <div>
                                                <h4 className="font-bold text-slate-400">{badge.name}</h4>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                        ...
                        <div className="space-y-4">
                            {MOCK_LEADERBOARD.map((user, idx) => (
                                <div
                                    key={idx}
                                    className={`flex items-center justify-between p-3 rounded-lg ${user.rank === 5 ? 'bg-blue-600/10 border border-blue-600/30' : 'hover:bg-slate-800'
                                        }`}
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <span className={user.rank === 5 ? 'text-blue-400 font-bold' : 'text-slate-300 font-medium'}>
                                            {user.name}
                                        </span>
                                    </div>
