...
const Quiz = () => {
    ...
    const handleNext = () => {
        ...
        const isCorrect = selectedOption === selectedQuiz.questions[currentQ].correctIndex;
    ...
    const saveQuizResult = (finalScore: number) => {
        const newResult = {
            id: Date.now(),
            quizId: selectedQuiz._id, // Use _id from API
            title: selectedQuiz.title,
            score: finalScore,
            total: selectedQuiz.questions.reduce((acc: number, q: any) => acc + q.marks, 0), // Calculate total marks
            ...
            <div className="max-w-4xl mx-auto">
                    ...
                    <div className="space-y-6">
                        ...
                        {loading ? (
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
                                <p className="text-slate-400 text-lg">No quizzes assigned yet.</p>
                            </div>
                        ) : (
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                {quizzes.map((quiz) => (
                                    <div key={quiz._id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-blue-500/50 transition-all cursor-pointer group"
                                        ...
                                        <h3 className="text-xl font-bold text-white mb-2 group-hover:text-blue-400 transition-colors">{quiz.title}</h3>
                                        <p className="text-slate-400 text-sm mb-4">{quiz.questions.length} Questions • {quiz.subjectCode}</p>
                                        <button className="w-full py-2 bg-slate-800 group-hover:bg-blue-600 text-white rounded-lg font-medium transition-colors">
                                            Start Quiz
                                        </button>
                                    </div>
                                ))}
                ...
                {selectedQuiz && !started && !finished && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center space-y-6 relative overflow-hidden group">
                            ...
                            <h1 className="text-3xl font-bold text-white relative z-10">{selectedQuiz.title}</h1>
                            <p className="text-slate-400 relative z-10">
                                Subject: <span className="text-slate-300 font-medium">{selectedQuiz.subjectCode}</span> •
                                Questions: <span className="text-slate-300 font-medium">{selectedQuiz.questions.length}</span>
                            </p>
                            ...
                            <button
                                ...
                                className="relative z-10 px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg transition-transform hover:scale-105 shadow-lg shadow-blue-600/20"
                            >
                                Start Quiz
                            </button>
                        </div>
                ...
                {started && !finished && selectedQuiz && (
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between items-center text-slate-400 text-sm font-medium uppercase tracking-wider">
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8">
                            <h2 className="text-xl font-bold text-white mb-6">
                                {selectedQuiz.questions[currentQ].questionText}
                            </h2>

                            <div className="space-y-3">
                                {selectedQuiz.questions[currentQ].options.map((opt: string, idx: number) => {
                                    const isSelected = selectedOption === idx;
                                    const isCorrect = selectedQuiz.questions[currentQ].correctIndex === idx;

                                    let btnClass = "bg-slate-950 border-slate-800 text-slate-300 hover:bg-slate-800";
                                    if (showExplanation) {
                            ...
                            {/* Feedback Section */}
                            {showExplanation && (
                                <div className="mt-6 p-4 rounded-lg bg-slate-950 border border-slate-800 animate-in fade-in slide-in-from-top-2">
                                    <div className="flex items-start gap-3">
                                        <div className="mt-1">
                                            {selectedOption === selectedQuiz.questions[currentQ].correctIndex
                                                ? <CheckCircle className="text-emerald-500" size={20} />
                                                : <XCircle className="text-red-500" size={20} />
                                            }
                                        </div>
                                        <div>
                                            <h4 className={`font-bold ${selectedOption === selectedQuiz.questions[currentQ].correctIndex ? 'text-emerald-400' : 'text-red-400'
                                                }`}>
                                                {selectedOption === selectedQuiz.questions[currentQ].correctIndex ? 'Correct Answer!' : 'Incorrect'}
                                            </h4>
                                    ...
                                    <button
                                        ...
                                        className="mt-4 w-full py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg font-medium transition-colors"
                                    >
                            ...
                            {!showExplanation && (
                                <button
                                    ...
                                    onClick={handleNext}
                                    className="mt-8 w-full py-3 bg-blue-600 disabled:bg-slate-800 disabled:text-slate-500 hover:bg-blue-500 text-white rounded-lg font-bold transition-all"
                                >
                                    Submit Answer
                                </button>
                            )}
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center space-y-6">
                        ...
                        <div className="flex justify-center gap-8 my-6">
                            <div className="text-center">
                                <div className="text-4xl font-bold text-white">{score} / {selectedQuiz.questions.reduce((acc: number, q: any) => acc + q.marks, 0)}</div>
                        ...
                        <div className="p-4 bg-slate-950 rounded-lg border border-slate-800">
                            <h3 className="text-slate-300 font-medium mb-2">Performance Summary</h3>
                        ...
                        <button
                            ...
                            className="px-6 py-2 border border-slate-700 hover:bg-slate-800 text-white rounded-lg transition-colors"
                        >
                            Back to Quizzes
                        </button>
                    </div>
