        ...
        <DashboardLayout role="parent">
            <div className="space-y-8">
                <div>
                    ...
                    <p className="text-slate-400">Track pending work and view graded submissions.</p>
                </div>

                <div className="flex gap-4 border-b border-slate-800 pb-1">
                    <button
                        onClick={() => setActiveTab('pending')}
                        className={`pb-3 px-2 text-sm font-medium transition-colors relative ${activeTab === 'pending' ? 'text-yellow-400' : 'text-slate-400 hover:text-white'}`}
                    >
                        Pending Assignments
                    ...
                    <button
                        ...
                        className={`pb-3 px-2 text-sm font-medium transition-colors relative ${activeTab === 'submitted' ? 'text-emerald-400' : 'text-slate-400 hover:text-white'}`}
                    >
                        View Submissions
                ...
                <div className="grid gap-4">
                    {activeTab === 'pending' ? (
                        PENDING_ASSIGNMENTS.map((assign) => (
                            <div key={assign.id} className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex items-center justify-between hover:border-yellow-500/30 transition-all">
                                <div className="flex items-start gap-4">
                                    <div className={`p-3 rounded-xl ${assign.status === 'Urgent' ? 'bg-red-500/10 text-red-400' : 'bg-blue-500/10 text-blue-400'}`}>
                                        <AlertCircle size={24} />
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-white text-lg">{assign.title}</h3>
                                        <div className="text-slate-400 text-sm mt-1">{assign.subject}</div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className={`text-sm font-bold ${assign.status === 'Urgent' ? 'text-red-400' : 'text-slate-300'}`}>
                                        Due {assign.dueDate}
                                    </div>
                                    <button className="mt-2 text-xs bg-slate-800 hover:bg-slate-700 text-white px-3 py-1 rounded transition-colors border border-slate-700">
                                        View Details
                                    </button>
                                </div>
                            </div>
                        ...
                        SUBMITTED_ASSIGNMENTS.map((assign) => (
                            <div key={assign.id} className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex items-center justify-between">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-white text-lg">{assign.title}</h3>
                                        <div className="text-slate-400 text-sm mt-1">{assign.subject} • Submitted on {assign.date}</div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    {assign.marks ? (
